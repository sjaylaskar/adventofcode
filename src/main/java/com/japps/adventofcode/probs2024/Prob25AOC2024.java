...
public final class Prob25AOC2024 extends AbstractSolvable implements Loggable {
	...
	private void compute() throws IOException {
		...
		fillLocksAndKeys(lines, locks, keys);
		println(locks.stream().mapToLong(lock -> keys.stream().filter(key -> isFitPair(lock, key)).count()).sum());
	}

	private void fillLocksAndKeys(List<String> lines, List<String> locks, List<String> keys) {
	...
	private String buildCombination(List<String> lockOrKey) {
		return IntStream.range(0, lockOrKey.getFirst().length()).mapToObj(columnIndex -> String.valueOf(lockOrKey.stream().filter(lockOrKeyLine -> FILLED_PATTERN == lockOrKeyLine.charAt(columnIndex)).count() - 1)).collect(Collectors.joining(StringUtils.EMPTY));
	}
}
